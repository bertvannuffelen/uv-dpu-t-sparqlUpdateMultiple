 ...
 */
@DPU.AsTransformer
public class SparqlUpdateMultiple extends AbstractDpu<SparqlUpdateConfig_V1> {

    private static final Logger LOG = LoggerFactory.getLogger(SparqlUpdateMultiple.class);
    ...
    protected void innerExecute() throws DPUException {
        if (useDataset()) {
            ContextUtils.sendShortInfo(ctx, "sparqlUpdateMultiple.dpu.mode.openRdf");
        } else {
            ContextUtils.sendShortInfo(ctx, "sparqlUpdateMultiple.dpu.mode.virtuoso");
        }

        this.dpuContext = this.ctx.getExecMasterContext().getDpuContext();	
	lastexec = this.dpuContext.getLastExecutionTime();

        // Get update query.
        final String query = config.getQuery();
        if (query == null || query.isEmpty()) {
            throw ContextUtils.dpuException(ctx, "sparqlUpdateMultiple.dpu.error.emptyQuery");
        }
        ...
        if (config.isPerGraph()) {
            ContextUtils.sendMessage(ctx, DPUContext.MessageType.INFO, "sparqlUpdateMultiple.dpu.info.perGraph",
                    "sparqlUpdateMultiple.dpu.info.perGraph.body", sourceEntries.size());
            ...
            if (sourceEntries.size() > MAX_GRAPH_COUNT) {
                throw ContextUtils.dpuException(ctx, "sparqlUpdateMultiple.dpu.error.tooManyGraphs",
            ...
            ContextUtils.sendShortInfo(ctx, "sparqlUpdateMultiple.dpu.info.singleOutput");
        ...
        ContextUtils.sendShortInfo(ctx, "sparqlUpdateMultiple.dpu.msg.report", inputSize, outputSize);
    }
    ...
    protected void updateEntries(final String updateQuery, final List<RDFDataUnit.Entry> sourceEntries,
        ...
        LOG.info("UpdateEntry Query: {}", updateQuery);
...
//                executeUpdateQuery(QUERY_COPY, toUriList(sourceEntries), targetgraph, connection);
                executeCopy(toUriList(sourceEntries), targetgraph, connection);
            }
		...
		for (String query : updateQuery.split("\\$")) {
                     LOG.info("UpdateEntry Query Split: {}", query);
                     executeUpdateQuery(query, Arrays.asList(targetgraph), targetgraph, connection);
		}
            }
...
/* taking into account the last execution date.
   It requires to do a replacement of the the last_execution_date with the lastdates value.

	DateFormat formatter = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
	String lastdates = "empty";
	if (lastexec == null) { lastdates = "not present"; } else {
	     lastdates = formatter.format(lastexec);
	}
        LOG.info("last execution time: {}", lastdates);
...
*/
        ...
        try {
            ...
            throw ContextUtils.dpuException(ctx, ex, "sparqlUpdateMultiple.dpu.error.query");
        } catch (RepositoryException ex) {
            throw ContextUtils.dpuException(ctx, ex, "sparqlUpdateMultiple.dpu.error.repository");
        }
    }
     ...
     * @throws eu.unifiedviews.dpu.DPUException
     */
    protected void executeCopy(List<URI> sourceGraphs, URI targetGraph,
            RepositoryConnection connection) throws DPUException {
        try {
            // Execute query.
		for (URI graph : sourceGraphs) {
			String query = "ADD <"+ graph.stringValue() + "> TO <" + targetGraph.stringValue() + ">";
			LOG.info("Query to execute: {}", query);
		    
		     Update update2 = connection.prepareUpdate(QueryLanguage.SPARQL, query);
		     update2.execute();
		}
        } catch (MalformedQueryException | UpdateExecutionException ex) {
            throw ContextUtils.dpuException(ctx, ex, "sparqlUpdateMultiple.dpu.error.query");
    ...
    protected URI createOutputGraph() throws DPUException {
        ...
        try {
            ...
            throw ContextUtils.dpuException(ctx, ex, "sparqlUpdateMultiple.dpu.error.cantAddGraph");
        }
    }
            ...
            public void action() throws Exception {
                for (RDFDataUnit.Entry entry : entries) {
                    try {
                        ...
                        throw ContextUtils.dpuException(ctx, ex, "sparqlUpdateMultiple.dpu.error.dataUnit");
                    }
                }
            }
    ...
    protected final boolean useDataset() {
        // Should be removed once bug in Sesame or Virtuoso is fixed.
